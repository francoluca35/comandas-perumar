...
export default function ModalMesa({ mesa, onClose, refetch }) {
  ...
  const imprimirTicket = (orden, hora, fecha) => {
    const nuevaVentana = window.open("", "Ticket", "width=384,height=600");
    ...
    const html = `
      <html>
        ...
        <body>
          ...
          <h2>Perú Mar</h2>
          <div>Mesa: <strong>${mesa.numero}</strong></div>
          <div>Orden #: ${orden}</div>
          <div>Hora: ${hora}</div>
          <div>Fecha: ${fecha}</div>
          <div class="line"></div>
  
          <div><strong>COMIDAS</strong></div>
          ${comidas
            .map(
              (p) => `
            <div class="item">
          ...
          <div style="margin-top: 6px;"><strong>BEBIDAS</strong></div>
          ${bebidas
            .map(
              (p) => `
            <div class="item">
          ...
          <div class="line"></div>
          <div class="item"><strong>Total</strong><strong>$${total.toFixed(
            2
          )}</strong></div>
          <div class="line"></div>
