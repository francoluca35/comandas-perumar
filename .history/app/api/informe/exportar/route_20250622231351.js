...
import { connectToDatabase } from "@/lib/connectToDatabase"; // Asegurate que este path sea correcto

export async function GET() {
  try {
    const db = await connectToDatabase();
    const ingresos = await db.collection("ingresodiarios").find().toArray();
    const retiros = await db.collection("retirosefectivos").find().toArray();
