  ...
  try {
    ...
    for (const ingreso of ingresos) {
      ...
      if (!agruparPorFecha[fecha]) {
        agruparPorFecha[fecha] = { ingreso: 0, retiros: 0 };
      }
    ...
    // ✅ Si se descarga informe mensual, eliminamos esos registros
    if (tipo === "mes") {
      const borrarIngresos = db.collection("ingresosDiarios").deleteMany({
        timestamp: {
          $gte: inicioMes,
          $lte: finMes,
        },
      ...
      const borrarRetiros = db.collection("retiroEfectivo").deleteMany({
        timestamp: {
          $gte: inicioMes,
          $lte: finMes,
        },
      ...
      await Promise.all([borrarIngresos, borrarRetiros]);
    }
