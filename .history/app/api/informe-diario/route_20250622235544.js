...
export async function GET() {
  try {
    ...
    for (const retiro of retiros) {
      const retiroDate = retiro.fecha || retiro.timestamp;
      if (!retiroDate) continue;

      const fechaISO = new Date(retiroDate).toISOString().split("T")[0];
      ...
      agruparPorFecha[fechaISO].retiros.push({
        hora: new Date(retiroDate).toLocaleTimeString("es-AR", {
          hour: "2-digit",
          minute: "2-digit",
        }),
    ...
    // Procesar retiros
    for (const retiro of retiros) {
      const fechaValida = retiro.fecha || retiro.timestamp;

      if (!fechaValida || isNaN(new Date(fechaValida).getTime())) continue;

      const fechaISO = new Date(fechaValida).toISOString().split("T")[0];
