...
export async function GET() {
  try {
    const db = await connectToDatabase(); // ya te devuelve el db
    ...
    const map = {}; // 🟨 ¡FALTABA ESTO!

    // Agrupar ingresos
    ingresos.forEach((ing) => {
      const fecha =
        ing.fecha || new Date(ing.timestamp).toISOString().split("T")[0];
      if (!fecha) return;
      if (!map[fecha]) {
        map[fecha] = {
          ingresoTotal: 0,
      ...
      map[fecha].ingresoTotal += ing.ingresoTotal || ing.totalPedido || 0;
    });
