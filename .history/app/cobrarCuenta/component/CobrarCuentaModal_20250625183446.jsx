...
export default function CobrarCuentaModal({
  ...
  const confirmarPago = async () => {
    if (metodo === "Efectivo" || metodo === "Mercado Pago") {
      // Imprime visualmente en pantalla (opcional)
      imprimirTicket();

      // Imprime físicamente en impresora térmica y abre la caja si es efectivo
      await fetch("http://localhost:4000/print-ticket-pago", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          mesa: mesa.numero || mesa, // depende de tu estructura
          total: totalFinal,
          metodoPago: metodo, // "Efectivo" o "Mercado Pago"
        }),
      });
