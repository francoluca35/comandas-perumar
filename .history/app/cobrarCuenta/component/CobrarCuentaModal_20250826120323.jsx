...
export default function CobrarCuentaModal({
  ...
  const [modoComision, setModoComision] = useState("sumar"); // "sumar" o "reemplazar"
  ...
  useEffect(() => {
    if (paso === "qr" || paso === "link") {
      if (modoComision === "sumar") {
        setTotalMP(
          Math.round((Number(totalFinal) + Number(comisionMP)) * 100) / 100
        );
      } else {
        // Modo reemplazar: usar directamente el valor de comisionMP como total
        setTotalMP(Number(comisionMP) || Number(totalFinal));
      }
    }
  }, [paso, totalFinal, comisionMP, modoComision]);
  ...
  }, [paso, externalReference, totalMP, comisionMP, modoComision, mesa.numero, productos]);
          ...
          <div className="mb-2">
            ...
            <div className="flex justify-center gap-2 mb-3">
              <label className="flex items-center gap-1 text-sm">
                <input
                  ...
                  name="modoComision"
                  value="sumar"
                  checked={modoComision === "sumar"}
                  onChange={(e) => setModoComision(e.target.value)}
                  className="mr-1"
                />
                Sumar comisión
              </label>
              <label className="flex items-center gap-1 text-sm">
                <input
                  ...
                  name="modoComision"
                  value="reemplazar"
                  checked={modoComision === "reemplazar"}
                  onChange={(e) => setModoComision(e.target.value)}
                  className="mr-1"
                />
                Reemplazar total
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <input
                ...
                placeholder={modoComision === "sumar" ? "0.00" : totalFinal.toFixed(2)}
              />
              <span className="text-gray-700 text-xs">
                {modoComision === "sumar" ? "(comisión)" : "(monto total)"}
              </span>
            </div>
            ...
            <div className="text-center mt-1">
              ...
              {modoComision === "sumar" && comisionMP > 0 && (
                <div className="text-xs text-gray-600">
                  (${totalFinal.toFixed(2)} + ${comisionMP.toFixed(2)} comisión)
                </div>
              )}
              {modoComision === "reemplazar" && comisionMP > 0 && (
                <div className="text-xs text-gray-600">
                  (reemplaza ${totalFinal.toFixed(2)})
                </div>
              )}
            </div>
          </div>
