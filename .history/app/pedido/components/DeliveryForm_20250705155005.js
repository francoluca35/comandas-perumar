...
export default function DeliveryForm() {
  ...
  const [comidaSeleccionada, setComidaSeleccionada] = useState(null); // Objeto comida
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        ...
        {/* Lista filtrada */}
        {comida && (
          <div className="max-h-48 overflow-y-auto bg-white/10 border border-white/20 rounded-xl mb-4">
            {comidas
              .filter((p) =>
                p.nombre.toLowerCase().includes(comida.toLowerCase())
              )
              .map((p) => (
                <div
                  key={p._id}
                  onClick={() => {
                    setComida("");
                    setAdicionalesDisponibles(p.adicionales || []);
                    setAdicionalesSeleccionados([]);
                    setPresupuesto((prev) => [
                      ...prev,
                      { comida: p.nombre, bebida: "", adicionales: [] },
                    ]);
                  }}
                  className="cursor-pointer px-4 py-2 text-white hover:bg-white/20 border-b border-white/10"
                >
                  {p.nombre}
                </div>
              ))}
