...
export default function CajaRetiro() {
  ...
  useEffect(() => {
    ...
    // Mostrar alerta para exportar informe diario
    Swal.fire({
      title: "¿Deseás descargar el Excel del informe diario?",
      text: "Esto eliminará todos los datos actuales del informe diario.",
      icon: "warning",
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch("/api/informe/exportar");
          if (!res.ok) throw new Error("Error al generar Excel");
          const blob = await res.blob();
          const url = URL.createObjectURL(blob);
          const a = document.createElement("a");
          a.href = url;
          a.download = `informe-diario.csv`; // ⚠ NO xlsx

          a.click();
          URL.revokeObjectURL(url);
          Swal.fire(
            "✅ Éxito",
            "Excel descargado y datos eliminados.",
            "success"
          );
          fetchInforme(); // actualizar visualización
        } catch (err) {
  ...
  const descargarExcel = async (tipo) => {
    try {
      ...
      const a = document.createElement("a");
      a.href = url;
      a.download = `retiros-${tipo}.csv`; // ⚠ NO xlsx
