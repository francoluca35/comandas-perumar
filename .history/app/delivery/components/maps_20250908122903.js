...
export default function Maps() {
  ...
  const handleEnviar = async (pedido) => {
    ...
    // Imprimir ticket HTML (como antes)
    ...
    try {
      ...
      // Preparar productos para el ticket final
      const productosParaImprimir = pedido.comidas.map((item) => ({
        nombre: item.comida || item.bebida,
        cantidad: item.cantidad || 1,
        observacion: item.observacion || "",
        categoria: item.categoria || "", // Agregar categoría si existe
      }));

      // Imprimir ticket final en impresora térmica
      await fetch("/api/print-final-delivery", {
        ...
        body: JSON.stringify({
          ...
          total: pedido.total,
          metodoPago: pedido.formaDePago,
          modo: pedido.tipo === "delivery" ? "envio" : "retiro",
        }),
      });

      // Imprimir ticket HTML nuevamente (como antes)
      imprimirTicketPOS(pedido);
      await refetch();
    } catch (err) {
